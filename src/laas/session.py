...
class Session:
    ...
    def decrypt_password(self, hex_cipher: str):
        ...
        dec_password = ""
        try:
            dec_password = rsa.decrypt(cipherpassword, self.__rsa_private).decode(
                "utf8"
            )
        except rsa.pkcs1.DecryptionError:
            raise AnotherKeyError(
                "It looks like your password is encrypted with a different RSA key."
            )
        else:
            return dec_password
